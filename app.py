...
SIMILARITY_THRESHOLD = 0.59  # Lowered threshold for better recall
    ...
    try:
        ...
        for chunk in discourse_chunks:
            try:
                ...
                logger.info(f"[Discourse] Sim: {similarity:.4f} | Chunk: {chunk['content'][:80]}")  # Debug
        ...
        for chunk in markdown_chunks:
            try:
                embedding = json.loads(chunk["embedding"])
                similarity = cosine_similarity(query_embedding, embedding)
                logger.info(f"[Markdown] Sim: {similarity:.4f} | Chunk: {chunk['content'][:80]}")  # Debug
        ...
        # === FALLBACK if no result is above threshold ===
        if len(results) == 0:
            logger.warning("No results passed the threshold. Returning top 3 closest matches from markdown.")
            ...
            for chunk in markdown_chunks:
                try:
                    ...
                    logger.info(f"[Fallback] Sim: {sim:.4f} | Chunk: {chunk['content'][:80]}")
                    temp_results.append({
                        "source": "markdown",
                        "id": chunk["id"],
                        "title": chunk["doc_title"],
                    ...
                    logger.error(f"Error processing fallback chunk: {e}")
                    continue
